package com.lcd.ui.arcface.faceserver;
...
import com.lcd.ui.arcface.FaceRegisterInfo;
import com.lcd.ui.arcface.ImageUtil;
...
public class FaceServer {
    ...
    public void clearFace(Context context, String strRegisterName){
        synchronized (this) {
            if (context == null || TextUtils.isEmpty(strRegisterName)) {
                return;
            }
            ...
            FaceRegisterInfo faceregisterinfoDelete = null;

            if (faceRegisterInfoList != null) {
                for (FaceRegisterInfo faceregisterinfo : faceRegisterInfoList){
                    if (faceregisterinfo.getName().equals(strRegisterName)){
                        faceregisterinfoDelete = faceregisterinfo;
                        break;
                    }
                }
            }

            if (faceregisterinfoDelete != null){
                faceRegisterInfoList.remove(faceregisterinfoDelete);
            }

            File featureFile = new File(ROOT_PATH + File.separator + SAVE_FEATURE_DIR + File.separator + strRegisterName);

            if (featureFile.exists()){
                featureFile.delete();
            }

            File imgFile = new File(ROOT_PATH + File.separator + SAVE_IMG_DIR + File.separator + strRegisterName + ".jpg");
